...
def get_confluence_page_by_title(title: str, space_key: Optional[str] = None) -> str:
    ...
    a single space via `space_key` (e.g. 'ENG'). Returns the cleaned full
    content of the matching page, or a list of candidates if the title is
...
def list_confluence_pages_in_space(space_key: str, limit: int = 25) -> str:
    """
    List pages in a Confluence space as 'id | title'. Useful when search
    queries don't match well and you want to browse what's available in a
    space (get the `space_key` from `list_confluence_spaces`).
    """
    cf = get_confluence_client()
...
def search_confluence_content(query: str, limit: int = 3, max_chars: int = 3000) -> str:
    """
    Search Confluence and return the full cleaned content of each matching
    page (heavy — multiple page bodies). Prefer `search_confluence_pages` +
    `get_confluence_page` for more concise answers; use this only when you
    need to compare several pages at once.
    """
    cf = get_confluence_client()
    safe = _escape_cql(query)
    results = cf.cql(f'text ~ "{safe}" AND type = page', limit=limit)
